...
class SessionGuard implements StatefulGuard, SupportsBasicAuth
{
    ...
    public function user()
    {
        ...
        //如果我们已经检索到当前请求的用户，我们可以立即返回它。我们不想每次调用这个方法来获取用户数据，因为这将非常缓慢。
        if (! is_null($this->user)) {
            return $this->user;
        }

        //登录后会有个登录session login_admin_59ba36addc2b2f9401580f014c7f58ea4e30989d
        //这里取得的名字就是在这个
        //然后用取得这个session的值,这个值貌似是用户id
        $id = $this->session->get($this->getName());

        // First we will try to load the user using the identifier in the session if
        // one exists. Otherwise we will check for a "remember me" cookie in this
        // request, and if one exists, attempt to retrieve the user using that.
        //首先，如果$id存在的话，我们将尝试在会话中使用标识符来加载用户。
        //否则，我们将检查“记住我”的cookie这一请求，如果存在，尝试让用户使用那个。
        $user = null;

        if (! is_null($id)) {
            //存在的话就读取用户信息
            //retrieveById: 该方法获取并返回匹配该ID的Authenticatabl 实现。
        ...
        // 如果用户是无效的，但我们解密“发件人”cookie
        //，我们可以尝试把用户数据在cookie作为记得在应用程式。
        //一旦我们有了用户，我们可以将它返回给调用方。
        //这里去尝试cookies中搜索 'remember_'.$this->name.'_'.sha1(static::class); remember这个值
        //因为如果点'记住我'的话会保存这个session
        $recaller = $this->getRecaller();

        //如果remember cookies存在
        if (is_null($user) && ! is_null($recaller)) {
            $user = $this->getUserByRecaller($recaller);

            if ($user) {
                $this->updateSession($user->getAuthIdentifier()); //更新session值

                $this->fireLoginEvent($user, true); //触发登录事件
            }
        }
